...
class AGCN(nn.Module):
    ...
    def __init__(self, in_channels, hidden_channels, out_channels,heads, num_layers, is_realtime=False):
        ...
        self.lin1 = Linear(hidden_channels * 3*heads, hidden_channels * 3)
        self.lin2 = Linear(hidden_channels * 3, out_channels)

        self.dropout = nn.Dropout(p=0.6)

        for _ in range(num_layers):
            conv1 = HeteroConv({('frame_window', 'spatial', 'frame_window'): GATConv(in_channels, hidden_channels,heads),
                                ('frame_window', 'temporal', 'frame_window'): GATConv(in_channels, hidden_channels,heads)},
                               aggr='sum')
            conv2 = HeteroConv({('frame_window', 'spatial', 'frame_window'): GATConv(hidden_channels*heads, hidden_channels,heads),
                                ('frame_window', 'temporal', 'frame_window'): GATConv(hidden_channels*heads,
                                                                                      hidden_channels,heads)}, aggr='sum')
            conv3 = HeteroConv({('frame_window', 'spatial', 'frame_window'): GATConv(hidden_channels*heads, hidden_channels,heads),
                                ('frame_window', 'temporal', 'frame_window'): GATConv(hidden_channels*heads,
                                                                                      hidden_channels,heads)}, aggr='sum')
            bn1 = nn.BatchNorm1d(hidden_channels*heads)
            bn2 = nn.BatchNorm1d(hidden_channels*heads)
            bn3 = nn.BatchNorm1d(hidden_channels*heads)
