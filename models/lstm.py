...
class LSTMGCN(nn.Module):
    ...
    def __init__(self, in_channels, hidden_channels, out_channels, num_layers, is_realtime=False):
        ...
        # Define LSTM aggregators
        self.lstm_aggregator1 = nn.LSTM(hidden_channels, hidden_channels, batch_first=True)
        self.lstm_aggregator2 = nn.LSTM(hidden_channels, hidden_channels, batch_first=True)
        self.lstm_aggregator3 = nn.LSTM(hidden_channels, hidden_channels, batch_first=True)
    ...
    def forward(self, data):
        ...
        for conv1, conv2, conv3, bn1, bn2, bn3 in zip(self.convs1, self.convs2, self.convs3,
            ...
            gc1_lstm, _ = self.lstm_aggregator1(gc1['frame_window'].unsqueeze(0))
            gc1_lstm = gc1_lstm.squeeze(0)
            ...
            gc2_lstm, _ = self.lstm_aggregator2(gc2['frame_window'].unsqueeze(0))
            gc2_lstm = gc2_lstm.squeeze(0)
        ...
        #             gc3 = {key: self.dropout(bn3(x.relu())) for key, x in gc3.items()}
            gc3_lstm, _ = self.lstm_aggregator3(gc3['frame_window'].unsqueeze(0))
            gc3_lstm = gc3_lstm.squeeze(0)

        out1 = global_mean_pool(gc3_lstm, batch)
        out2 = global_add_pool(gc3['frame_window'], batch)
        out3 = global_max_pool(gc3_lstm, batch)
